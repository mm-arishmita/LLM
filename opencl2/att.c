...
/* Number of Compute Elements used for parallel partial dot products. */
#define CE_USE_NUM      1
                        ...
                        __Op32 base,     __Op32 bound,   __Op32 reduceSumFr)
{
    ...
    float acc = 0.0;
    for (int d = lo; d < hi; d++) {
        acc += (float)g_Q[IDX_Q(h,d)] * (float)g_K[IDX_K(t,d)];
    }
...
__hyperOp__ void ReduceSum(__Op32 head_id, __Op32 tok_id, __Op32 endFr)
{
    ...
    float dot = 0.0;
    for (int ce = 0; ce < CE_USE_NUM; ce++) {
        dot += (float)g_partials[IDX_P(h,t,ce)];
    }

    /* Scale and write final score — unchanged */
    float inv_sqrt_dim = 1.0f / custom_sqrtf((float)HEAD_DIM);
    g_Scores[IDX_S(h,t)] = (float)(dot * (float)inv_sqrt_dim);
