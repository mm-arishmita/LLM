 ...
 *  build time — same approach as wsum.c/rms.c. Must match the values in
 *  common_defs.h used by the host.)
 * =============================================================================*/
 ...
 * (Captured from host buffers at softmax_start() entry. Flat-indexed since
 *  OpenCL kernel arguments arrive as raw mat_t* rather than fixed-size
 *  multi-dimensional arrays.)
 * =============================================================================*/
static mat_t *g_score;
static mat_t *g_output;

#define IDX_BHQK(b,h,q,k)  ((((b) * HEAD_NUM + (h)) * Q_LEN + (q)) * K_LEN + (k))

/* =============================================================================
 * SCRATCH BUFFER (device-internal only — never written/read by host)
 * =============================================================================*/
...
__hyperOp__ void FindMax(__Op32 batch_id, __Op32 head_id, __Op32 query_id,
  ...
  float max_val = g_score[IDX_BHQK(b,h,q,0)];
  for(int i = 1; i < K_LEN; i++) {
    if(g_score[IDX_BHQK(b,h,q,i)] > max_val) {
      max_val = g_score[IDX_BHQK(b,h,q,i)];
    }
  }
...
__hyperOp__ void Exponent(__Op32 batch_id, __Op32 head_id, __Op32 query_id,
  ...
  for(int k = (int)base.i32; k < (int)bound.i32; k++) {
    exp_vec[IDX_BHQK(batch,head,query,k)] =
        EXP(g_score[IDX_BHQK(batch,head,query,k)] - max_val.f32);
  }
...
__hyperOp__ void Sum(__Op32 batch_id, __Op32 head_id, __Op32 query_id,
  ...
  for(int k = 0; k < K_LEN; k++) {
    exp_sum += exp_vec[IDX_BHQK(batch,head,query,k)];
  }
...
__hyperOp__ void Softmax(__Op32 batch_id, __Op32 head_id, __Op32 query_id,
  ...
  for(int k = (int)base.i32; k < (int)bound.i32; k++) {
    g_output[IDX_BHQK(batch,head,query,k)] =
        exp_vec[IDX_BHQK(batch,head,query,k)] * exp_scale.f32;
  }
...
/// End HyperOp.
__hyperOp__ void End() {
    re_println("End HyperOp — Softmax complete.\n");
    CE_FLUSH_ALL();
    LLC_FLUSH_ALL();
...
/* =============================================================================
 * softmax_start - FPGA ENTRY POINT
 * =============================================================================*/
__kernel void softmax_start(mat_t *score,
                            mat_t *output)
{
